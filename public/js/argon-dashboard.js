...
function sidebarColor(element) {
  ...
  // Menghapus kelas 'active' dari semua elemen sibling
  ...
  // Menerapkan warna pada sidebar
  var sidebar = document.querySelector('.sidenav');
  sidebar.setAttribute("data-color", color);

  // Menerapkan warna pada card (jika ada)
  ...
  console.log("Menyimpan warna:", color);  // Log ini untuk memastikan warna yang akan disimpan
  // Simpan warna ke localStorage
  localStorage.setItem("sidebarColor", color);
}

// Mengambil warna dari localStorage saat halaman dimuat
window.addEventListener("load", function() {
  var savedColor = localStorage.getItem("sidebarColor");
  console.log("Warna yang diambil dari localStorage:", savedColor);  // Log ini untuk melihat warna yang diambil
  if (savedColor) {
    // Terapkan warna yang tersimpan pada sidebar
    var sidebar = document.querySelector('.sidenav');
    sidebar.setAttribute("data-color", savedColor);

    // Terapkan warna yang tersimpan pada card (jika ada)
    if (document.querySelector('#sidenavCard')) {
      var sidenavCard = document.querySelector('#sidenavCard+.btn+.btn');
      let sidenavCardClasses = ['btn', 'btn-sm', 'w-100', 'mb-0', 'bg-gradient-' + savedColor];
      sidenavCard.removeAttribute('class');
      sidenavCard.classList.add(...sidenavCardClasses);
    }

    // Menambahkan kelas 'active' pada elemen yang sesuai
    var colorButtons = document.querySelectorAll('.badge.filter');
