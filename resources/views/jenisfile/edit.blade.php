        ...
        <div class="container">
          <form action="/jenis/{{ $jenis->id }}" method="POST">
            ...
            <div class="form-group">
              ...
              <input type="text" class="form-control" id="nomor" placeholder="DC06" name="nomor" value="{{ old('nomor',$jenis->nomor) }}" required autofocus>
              @error('nomor')
            ...
            <div class="form-group">
              ...
              <select class="form-control @error('jenis') is-invalid @enderror" aria-label=".form-select-sm example" name="jenis" id="jenis" required>
                <option @if (old('jenis',$jenis->jenis)=="Alat") selected @endif value="Alat">Alat</option>
                <option @if (old('jenis',$jenis->jenis)=="Suku Cadang") selected @endif value="Suku Cadang">Suku Cadang</option>
              </select>
              @error('jenis')
                      <div class="alert alert-danger" style="color: white">{{ $message }}</div>
              @enderror
