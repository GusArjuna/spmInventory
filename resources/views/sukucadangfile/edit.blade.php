        ...
        <div class="container">
          <form action="/sukucadang/{{ $sukuCadang->id }}" method="POST">
            ...
            <div class="form-group">
              ...
              <input type="number" class="form-control @error('holdingCosts') is-invalid @enderror" id="holdingCosts" placeholder="500" name="holdingCosts" value="{{ old('holdingCosts',$sukuCadang->holdingCosts) }}" required>
              @error('holdingCosts')
              <div class="alert alert-danger" style="color: white">{{ $message }}</div>
              @enderror
            </div>
            <div class="form-group">
              ...
              <input type="number" class="form-control @error('biayaPemesanan') is-invalid @enderror" id="biayaPemesanan" placeholder="15000" name="biayaPemesanan" value="{{ old('biayaPemesanan',$sukuCadang->biayaPemesanan) }}" required>
            ...
            <div class="form-group">
              ...
              <select class="form-control @error('jenis') is-invalid @enderror" aria-label=".form-select-sm example" name="jenis" id="jenis" required>
                ...
                @foreach ($alats as $alat)
                  <option {{ (old('jenis',$sukuCadang->jenis)==$alat->nomor)?"selected":"" }} value="{{ $alat->nomor }}">{{ $alat->nomor }}</option>
                @endforeach
              </select>
            </div>
